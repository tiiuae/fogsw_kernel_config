...
if [ "${KERNEL_CONFIG}" == "fog" ]; then
    ...
    scripts/config --file /${BUILD_DIR}/${LINUX_SRC}/arch/x86/configs/${KERNEL_CONFIG}_defconfig \
        --set-str SYSTEM_TRUSTED_KEYS ""
    scripts/config --file /${BUILD_DIR}/${LINUX_SRC}/arch/x86/configs/${KERNEL_CONFIG}_defconfig \
        --set-str SYSTEM_REVOCATION_KEYS ""
    scripts/config --file /${BUILD_DIR}/${LINUX_SRC}/arch/x86/configs/${KERNEL_CONFIG}_defconfig \
        --enable ATH_REG_DYNAMIC_USER_REG_HINTS
    scripts/config --file /${BUILD_DIR}/${LINUX_SRC}/arch/x86/configs/${KERNEL_CONFIG}_defconfig \
        --enable ATH_REG_DYNAMIC_USER_CERT_TESTING
    scripts/config --file /${BUILD_DIR}/${LINUX_SRC}/arch/x86/configs/${KERNEL_CONFIG}_defconfig \
